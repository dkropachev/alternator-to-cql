 ...
 *
 * <p>Uses a conditional expression to prevent older records from overwriting newer ones:
 * the put succeeds only if the row does not exist or the existing timestamp is older than
 * the incoming one.
 */
public class DynamoDbWriter implements ItemWriter {

  private static final Map<String, String> EXPRESSION_ATTRIBUTE_NAMES =
      Map.of("#k", "id", "#l", "last_updated_millis");
  ...
  public DynamoDbWriter(DynamoDbClient dynamoClient, String tableName) {
  ...
  public void write(Map<String, AttributeValue> item) {
    AttributeValue tsValue = item.get("last_updated_millis");
    if (tsValue == null || tsValue.n() == null) {
      throw new IllegalArgumentException(
          "Item does not contain numeric attribute 'last_updated_millis'");
    }
